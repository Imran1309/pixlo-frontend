...
const PhotographerAvailability = () => {
        ...
        const loadExisting = async () => {
            ...
            try {
                ...
                if (p && p.availability) {
                    ...
                    const newMorning = dayKeys.map((d) => (avail[d] || []).includes("morning"));
                    const newAfternoon = dayKeys.map((d) => (avail[d] || []).includes("afternoon"));
                    const newEvening = dayKeys.map((d) => (avail[d] || []).includes("evening"));

                    if (newMorning.some(Boolean) || newAfternoon.some(Boolean) || newEvening.some(Boolean)) {
                        setAvailability({ morning: newMorning, afternoon: newAfternoon, evening: newEvening });
                    }
                }
            } catch (err) {
    ...
    const handlePublish = async () => {
        ...
        try {
            ...
            alert("🎉 Profile Published! Your profile is now live for clients to book you.");
            navigate("/");
        } catch (err) {
            console.error("Error publishing availability:", err);
            alert("Failed to save availability. Your profile may still be incomplete.");
