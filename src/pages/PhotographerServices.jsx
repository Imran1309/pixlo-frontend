...
const PhotographerServices = () => {
    ...
    const [services, setServices] = useState([
        { id: Date.now(), name: "", description: "", duration: 1, price: 0, deliverables: "" },
    ]);
    ...
    useEffect(() => {
        const loadExisting = async () => {
            ...
            try {
                const res = await axios.get(`${API_URL}/api/photographers/profile/${user._id}`);
                const photographer = res.data.photographer;
                if (photographer && photographer.services && photographer.services.length > 0) {
    ...
    if (loadingExisting) {
        return (
            <div className="services-page" style={{ display: "flex", alignItems: "center", justifyContent: "center", height: "100vh" }}>
                <div style={{ color: "#fff", fontSize: "1.2rem" }}>Loading your services...</div>
            </div>
        );
    }
        ...
        <div className="services-page">
            ...
            <div className="services-content">
                                ...
                                <div className="input-group">
                                    ...
                                    <div className="number-input-container">
                                        ...
                                        <div className="spinner-btns">
                                            <button className="spinner-btn" onClick={() => handleInputChange(service.id, "duration", Number(service.duration) + 0.5)}>▲</button>
                                            <button className="spinner-btn" onClick={() => handleInputChange(service.id, "duration", Math.max(0.5, Number(service.duration) - 0.5))}>▼</button>
                                        </div>
                                    </div>
                                </div>
                                <div className="input-group">
                ...
                <div className="service-footer">
                    <button className="btn-back" onClick={handleBack} style={{ background: "#fff", color: "#000", border: "none", padding: "0.8rem 2rem", borderRadius: "30px", fontWeight: "600", cursor: "pointer" }}>
                        Back
                    </button>
                    <button
                        ...
                        disabled={saving}
                        style={{ background: saving ? "#888" : "#CBB26A", color: "#000", border: "none", padding: "0.8rem 2.5rem", borderRadius: "30px", fontWeight: "600", cursor: saving ? "not-allowed" : "pointer" }}
                    >
