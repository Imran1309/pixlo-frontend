...
const FindCreators = () => {
  ...
  useEffect(() => {
    const fallbackToProfile = () => {
      ...
      if (storedUser && storedUser.latitude && storedUser.longitude) {
        setUserLocation({
          lat: Number(storedUser.latitude),
          lng: Number(storedUser.longitude),
        });
        setLocationError(null);
      } else {
        setLocationError(
          "Location access denied. Please update your City in Profile.",
        );
      }
    };
  ...
  const filteredPhotographers = photographers.filter((p) => {
    if (selectedFilters.categories.length > 0) {
      const hasCategory = selectedFilters.categories.some(
        (cat) =>
          ...
          (p.services &&
            p.services.some((s) =>
              s.serviceName?.toLowerCase().includes(cat.toLowerCase()),
            )),
      );
    ...
    if (
      ...
      (p.startingPrice || 0) > selectedFilters.priceRange
    ) {
      return false;
    }
                ...
                <div key={p._id} className="creator-card-new">
                  ...
                  <div className="card-details">
                    ...
                    <div className="card-tags">
                      {p.specialization &&
                        p.specialization.slice(0, 2).map((s) => (
